...
def river_reward(
    ...
    current_path_length, longest_path = calc_longest_path(water_binary_map)
    ...
    if hard:
        region_reward = -abs(number_of_regions - 1)
        path_reward = -abs(current_path_length - target_river_length)
    else:
        region_reward = 1 - number_of_regions
        path_reward = min(current_path_length - target_river_length, 0)
    
    land_region_reward = -max(number_of_land_regions - max_land_regions, 0)
    water_center_penalty = -number_of_water_centers
    
    total_reward = region_reward + path_reward + water_center_penalty + land_region_reward
    ...
    info = {
        ...
        "target_river_length": target_river_length,
        ...
        "land_region_reward": land_region_reward,
    }
    
    return min(total_reward, MAX_RIVER_REWARD), info


def get_river_biome(grid: list[list[set[str]]]) -> str:
    water_tiles = {
        "water", "water_tl", "water_tr", "water_t", "water_l", "water_r", "water_bl", "water_b", "water_br", "shore_tl", "shore_tr", "shore_bl", "shore_br", "shore_lr", "shore_rl"
    }
