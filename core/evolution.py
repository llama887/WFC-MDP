    ...
    def run_action_sequence(self):
        ...
        if self.genotype_representation == "1d":
            for idx, action in enumerate(self.action_sequence):
                ...
                self.reward += reward
                applied_index_order.append(idx)
                self.info = info
                if terminate or truncate:
                    break

        elif self.genotype_representation == "2d":
            terminate = False
            truncate = False
            while not (terminate or truncate):
                ...
                self.reward += reward
                applied_index_order.append(flattened_index)
                self.info = info

        end_time = timer()
        self.info["wfc_rollout_time"] = end_time - start_time
        self.info["applied_index_order"] = applied_index_order
...
if __name__ == "__main__":
    ...
    if best_agent:
        ...
        applied = best_agent.info.get("applied_index_order")
        ...
        if applied:  # preferred exact replay path
            for idx in applied:
                action = best_agent.action_sequence[idx]
            ...
            # Fallback: mirror evaluation logic based on genotype
            if best_agent.genotype_representation == "2d":
                terminated = truncated = False
                while not (terminated or truncated):
                    next_collapse_x, next_collapse_y = map(int, observation[-2:])
                    flattened_index = next_collapse_y * env.map_width + next_collapse_x
                    action = best_agent.action_sequence[flattened_index]
                    observation, reward, terminated, truncated, info = env.step(action)
                    total_reward += reward
                    env.render()
                    pygame.display.flip()
